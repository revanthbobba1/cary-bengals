...
| `031_list_league_members.sql` | Phase 3a: `list_league_members()` — `SECURITY DEFINER` function powering the commissioner's "assign an article" member dropdown, since `auth.users` isn't exposed to PostgREST. Same pattern as `017`/`018`. |
| `032_save_article_matchups.sql` | Phase 3b: `save_article_matchups()` — the editor's "Save" action. Deletes and re-inserts an article's full matchup list in one transaction rather than diffing individual rows, the same pattern `submit_poll_ballot` (020) uses for poll rankings — necessary because `article_matchups`' `UNIQUE (article_id, position)` isn't deferrable, so per-row updates would collide on any reorder. |
| `033_lock_article_row_on_save_matchups.sql` | `CREATE OR REPLACE` on `save_article_matchups` closing a TOCTOU gap: the ownership/draft check used a non-locking `SELECT EXISTS`, so a concurrent `publish_article()` could commit between the check and the DELETE+INSERT, landing matchup edits on an already-published article without `publish_article()`'s completeness validation. Same class of bug `023` fixed for `submit_poll_ballot`; same fix (`SELECT ... FOR UPDATE`). |
